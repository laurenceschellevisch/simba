...
var 
  ...
  tmp:array of Byte;
begin
  tmp := self.scan.CopyMem(address+W_SIZE_OFFSET,4,False);
  data := BytesToInt( tmp );
  Result := data = self.bufferW * self.bufferH;
  
  SetLength(tmp, 0); // Leak fix - Simba1.2rc6
end;
...
begin
  ...
  begin
    tmp := scan.CopyMem(matches[i],48);
    TIA := BytesToTIA( tmp );
    SetLength(tmp, 0);   //leakfix - Simba 1.2rc6
  ...
  Setlength(matches, 0);  //leakfix - Simba1.2rc6
...
begin
  ...
  SetLength(tpa, 0);  // Leak fixes - Simba1.2rc6
  SetLength(acc, 0);  //
end;
...
begin
  ...
  SetLength(mat, 0); // Leak fix Simba1.2rc6
end;
...
begin
  ...
  SetLength(tmp1, 0);  //leakfix - Simba1.2rc6
  SetLength(tmp2, 0);  //
  SetLength(world,0);  //
end;
